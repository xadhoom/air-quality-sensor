...
class Loop:
    ...
    def schedule(self, hz: float, coroutine_function, immediate=True, *args, **kwargs):
        """
    ...
    def _step(self):
        ...
        if len(self._tasks) == 0 and len(self._sleeping) > 0:
            ...
            if sleep_seconds > 0:
                ...
                self._debug('No active tasks.  Sleeping for ',
                            sleep_seconds, 's. \n', self._sleeping)
    ...
    async def _sleep_until(self, target_run_time):
        ...
        # heap would be better but hey.
        self._sleeping.sort(key=Sleeper.resume_time)
